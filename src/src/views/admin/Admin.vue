            ...
            <a-layout class="profileBox">
                ...
                <a-layout-content :style="{ padding: '0px', minHeight: '280px', }" v-if="sider_status==1">
                    ...
                    <a-input-group compact class="paperSearchInput">
                        ...
                        <a-input-search v-model="searchPaperValue" style="width: 80%" placeholder="检索论文" size="large"
                                        enter-button @search="searchPaper"/>
                    </a-input-group>

                    <a-divider style="margin-bottom:0px"/>
                    <a-spin tip="加载中..." style="margin:auto" v-if="loading==true"></a-spin>
                    <a-list style="margin:0px 30px 0 30px" item-layout="vertical" size="large" :pagination="pagination"
                            :data-source="paper_list">
                        <a-list-item slot="renderItem" slot-scope="paper_list, index" :key="index">
                            ...
                            <div name="aaa" style="text-align:left; width:80%; padding:0px 0px 0px 20px;">
                                <p style="font-weight:700;">
                                    <a-icon type="book"/>&#12288;
                                    <span class="paperLink" v-if="paper_list.hasOwnProperty('url')"
                                          @click="paperLink(paper_list.url[0])">{{paper_list.title}}</span>
                                    <span v-if="paper_list.hasOwnProperty('url')==false">{{paper_list.title}}</span>
                                </p>
                                <p v-if="paper_list.hasOwnProperty('venue')"
                                   style="font-family:Times New Roman;font-weight:100;">{{paper_list.venue.name}}</p>
                ...
                <a-layout-content v-if="sider_status==2">
                    ...
                    <a-input-group compact class="paperSearchInput">
                        ...
                        <a-input-search v-model="searchAuthorPaperValue" style="width: 80%" placeholder="检索此作者的所有论文"
                                        size="large" enter-button @search="searchAuthorPaper"/>
                    </a-input-group>

                    <a-divider style="margin-bottom:0px"/>
                    <a-spin tip="加载中..." style="margin:auto" v-if="loading==true"></a-spin>
                    <a-list style="margin:0px 30px 0 30px" item-layout="vertical" size="large" :pagination="pagination"
                            :data-source="paper_author_list">
                        <a-list-item slot="renderItem" slot-scope="paper_author_list, index" :key="index">
                            <div name="aaa" style="text-align:left; width:80%; padding:0px 0px 0px 20px;">
                                <p style="font-weight:700;">
                                    <a-icon type="book"/>&#12288;
                                    <span class="paperLink" v-if="paper_author_list.hasOwnProperty('url')"
                                          @click="paperLink(paper_author_list.url[0])">{{paper_author_list.title}}</span>
                                    <span v-if="paper_author_list.hasOwnProperty('url')==false">{{paper_author_list.title}}</span>
                                </p>
                                <p v-if="paper_author_list.hasOwnProperty('venue')"
                                   style="font-family:Times New Roman;font-weight:100;">
                                    {{paper_author_list.venue.name}}</p>
                                <p style="margin-top:3px;font-weight:100;font-family:Times New Roman;font-size:14px">
                                    <template v-for="(author,authorIndex) in paper_author_list.authors">
                                        {{author.name}}
                                        <template v-if="authorIndex < paper_author_list.authors.length-1">{{', '}}
                                        </template>
                                    </template>
                                </p>
                            </div>
                ...
                <a-layout-content v-if="sider_status==3">
                    ...
                    <div style="text-align:center">
                        <a-table :columns="columns" :data-source="user_list">
                            <a class="usernameLink" slot="username" slot-scope="username" @click="goToUser">{{ username
                                }}</a>
